...
void Logger::setLogLevel(uint8_t level){
    ...
    if (_syslog) _syslog->logMask(_logMask);
}
...
bool Logger::logf(uint16_t pri, const char *fmt, ...) {
    ...
    if ((LOG_MASK(LOG_PRI(pri)) & this->_logMask) == 0) return true;

    va_start(args, fmt);
    char* msg = formatMessage(fmt,args);
    
    if (_syslog) result = _syslog->log(pri,msg);
    if (_serial) _serial->println(msg);
    va_end(args);
